package httpapi // HTTP API идентично rest-api по контракту; источник данных — MySQL
...
import (
	...
	"github.com/example/go-examples/mysql-ms/internal/application"
...
func (h *Handlers) Routes() chi.Router {
	r := chi.NewRouter()
	r.Post("/orders", h.createOrder) // тело JSON → CreateOrder → INSERT
	r.Get("/orders/{id}", h.getOrder) // SELECT по id
	return r
}
